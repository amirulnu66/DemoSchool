			...
			<div class="page-breadcrum">
				<div class="container">
					...
					@if(!empty($teacherMessage))	
						...
						<li>-</li>
						<li>{{$teacherMessage->breadcamp}}</li>
					@endif	
					</ul>
				</div> <!-- /.container -->
			</div> <!-- /.page-breadcrum -->
	        		...
	        		<div class="row">
	        			...
	        			@if(!empty($teacherMessage))	
	        				<div class="text-warpper">
	        					<h4>{{$teacherMessage->heading_one}}</h4>
	        					<p class="mmfix"> {{$teacherMessage->paragraph_one}}</p>
	        					<div class="row">
	        						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
	        							<h4>{{$teacherMessage->heading_two}}</h4>
	        							<p>{{$teacherMessage->paragraph_two}}</p>
	        						</div> <!-- /.col- -->
	        					</div>
	        				</div> <!-- /.text-wrapper -->
	        			</div>

	        			@php $teacher_info=json_decode($teacherMessage->teacher_info, true);  @endphp


	        			<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 teacher-sidebar float-left wow fadeInUp">
	        				<div class="img-holder theme-bg-color"><img src="{{url('asset/uploads/'.$teacherMessage->profile_picture)}}" alt="Teacher"></div>
